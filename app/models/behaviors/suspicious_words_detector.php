    ...
    function beforeSave(&$Model) 
    {
        ...
        if (!empty($this->__settings[$Model->alias]['fields']) && $this->__badWords) {
            ...
            if (!empty($is_system_flagged)) {
                $Model->data[$Model->name]['is_system_flagged'] = 1;
                if (Configure::read('coupon.auto_suspend_coupon_on_system_flag') || Configure::read('store.auto_suspend_store_on_system_flag') ) { // During auto suspend turned on by admin
                    $Model->data[$Model->name]['admin_suspend'] = 1;
                }

                $Model->data[$Model->name]['detected_suspicious_words'] = serialize(array_unique($this->__detected_suspicious_words));
            } else { // During Edit
                $Model->data[$Model->name]['detected_suspicious_words'] = '';
                $Model->data[$Model->name]['is_system_flagged'] = 0;
            }
        }
       return true;
    }
